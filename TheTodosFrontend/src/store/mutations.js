...
export default {
    loadLists(state){
        axios.get('http://127.0.0.1:8000/api/lists').then(response => {
    ...
    updateTask(_, payload){
        ...
        if (id === undefined){
            axios.post('http://127.0.0.1:8000/api/tasks/', taskDetails);
        }
        else{
            axios.put(`http://127.0.0.1:8000/api/tasks/${id}`, taskDetails);
        }
    },
